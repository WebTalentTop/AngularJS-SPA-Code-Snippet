(function (global) {
    ...
    global.angular.module('cgBusy', []);

    global.angular.module('cgBusy').factory('_cgBusyTrackerFactory',['$timeout','$q',function($timeout,$q){
        ...
        return function(){
            ...
            tracker.reset = function(options){
                ...
                tracker.promises = [];
                global.angular.forEach(options.promises, function (p) {
    ...
    global.angular.module('cgBusy').value('cgBusyDefaults',{});

    global.angular.module('cgBusy').directive('cgBusy',['$compile','$templateCache','cgBusyDefaults','$http','_cgBusyTrackerFactory',
        function($compile,$templateCache,cgBusyDefaults,$http,_cgBusyTrackerFactory){
            return {
                ...
                link: function(scope, element, attrs, fn) {
                    ...
                    global.angular.extend(defaults, cgBusyDefaults);
                    ...
                    scope.$watchCollection(attrs.cgBusy,function(options){
                        ...
                        if (global.angular.isString(options)) {
                        ...
                        if (global.angular.isArray(options) || tracker.isPromise(options)) {
                            options = {promise:options};
                        }

                        options = global.angular.extend(global.angular.copy(defaults), options);

                        if (!options.templateUrl){
                            options.templateUrl = defaults.templateUrl;
                        }

                        if (!global.angular.isArray(options.promise)) {
                        ...
                        if (!global.angular.equals(tracker.promises, options.promise)) {
                            tracker.reset({
                        ...
                        if (!templateElement || currentTemplate !== options.templateUrl || backdrop !== options.backdrop) {
                            ...
                            $http.get(currentTemplate,{cache: $templateCache}).success(function(indicatorTemplate){
                                ...
                                global.angular.element(templateElement.children()[0])
    ...
    global.angular.module('cgBusy').run(['$templateCache', function($templateCache) {
      'use strict';
