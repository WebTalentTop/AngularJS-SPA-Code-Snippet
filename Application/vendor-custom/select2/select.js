    ...
    .controller('uiSelectCtrl',
      ...
      function ($scope, $element, $timeout, $filter, RepeatParser, uiSelectMinErr, uiSelectConfig) {
          ...
          _searchInput.on('keyup', function (e) {
              ctrl.userCallbacks && ctrl.userCallbacks['keyup'] && ctrl.userCallbacks['keyup'](ctrl.search);
          ...
          return {
              ...
              link: function (scope, element, attrs, ctrls, transcludeFn) {
                  ...
                  scope.$watch('callbacks', function () {
                      $select.userCallbacks = scope.$eval(attrs.callbacks) || {};
                  });
...
angular.module("ui.select").run(["$templateCache", function ($templateCache) {
    ...
    $templateCache.put("select2/choices.tpl.html", "<ul class=\"ui-select-choices ui-select-choices-content select2-results\"><li class=\"select2-no-results\" data-ng-show=\"$select.items.length===0\">No matches found</li><li class=\"ui-select-choices-group\" ng-class=\"{\'select2-result-with-children\': $select.isGrouped}\"><div ng-show=\"$select.isGrouped\" class=\"ui-select-choices-group-label select2-result-label\">{{$group.name}}</div><ul ng-class=\"{\'select2-result-sub\': $select.isGrouped, \'select2-result-single\': !$select.isGrouped}\"><li class=\"ui-select-choices-row\" ng-class=\"{\'select2-highlighted\': $select.isActive(this), \'select2-disabled\': $select.isDisabled(this)}\"><div class=\"select2-result-label ui-select-choices-row-inner\"></div></li></ul></li></ul>");
    ...
    $templateCache.put("select2/match.tpl.html", "<a class=\"select2-choice ui-select-match\" ng-class=\"{\'select2-default\': $select.isEmpty()}\" ng-click=\"$select.activate()\"><span ng-show=\"$select.searchEnabled && $select.isEmpty()\" class=\"select2-chosen\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" class=\"select2-chosen\" ng-transclude=\"\"></span> <abbr ng-if=\"$select.allowClear && !$select.isEmpty()\" class=\"select2-search-choice-close\" ng-click=\"$select.select(undefined)\"></abbr> <span class=\"select2-arrow ui-select-toggle\" ng-click=\"$select.toggle($event)\"><b></b></span></a>");
